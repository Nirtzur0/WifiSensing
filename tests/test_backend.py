...
def test_get_v_matrix(pcap_path, station_address, num_to_process):
    """
    ...
    # Run extraction
    timestamps, v_matrices = csi_backend.get_v_matrix(
        pcap_path,
        station_address,
    ...
    if len(v_matrices) == 0:
        pytest.skip(
            "No packets extracted from the PCAP file with the current filter. "
            "Set WIFI_SENSING_TEST_STATION_ADDRESS to an address present in the capture."
        )

    # Typical values specific to this dataset might be known, e.g., 64 subcarriers usually.
    # Keep assertions generic so the test remains robust across captures.
